...
void Game::update()
{
	...
	case GAME: // Must update all game objects when in the game, and also check for FSM updates
		...
		if (getCurrentLevel() == 3) // If on final level
		{
			...
			if (m_pPlayer2->getKeyEnemy()) // Boss is key enemy
			{
				GetFSM().pushState(new WinState); // If boss defeated, player wins game
			}
		}
...
void Game::updateGameObjects()
{
	...
	if (m_pExplosion->getExplosion() && m_pExplosion->hitBox)
	{
		...
		Collision::basicCollisionCheck(m_pExplosion, m_pPlayer2);
	...
	for (auto& currentEnemy : m_pEnemyVec)
	{
		...
		Collision::basicCollisionCheck(m_pPlayer2, currentEnemy);
	}
	...
	for (auto& currentPowerup : m_pPowerupVec)
	{
		...
		if (Collision::basicCollisionCheck(m_pPlayer2, currentPowerup) && !Collision::tileCollisionCheck(m_pPlayer2, getActiveMap()->currentMap))
		{
			currentPowerup->setCollected();
		}
	...
	m_pPlayer2->setIsColliding(Collision::tileCollisionCheck(m_pPlayer2, getActiveMap()->currentMap));
	if (m_pPlayer2->getIsColliding())
	{
		m_pPlayer2->setIsColliding(Collision::playerCollisionMovement(m_pPlayer2, getActiveMap()->currentMap));
	}
